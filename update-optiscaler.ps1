...
function Get-DownloadedAndExtracted {
...
try {
    ...
    if ($Force -or -not (Test-Path $targetDll)) {
        $nvngxPath = Get-DownloadedAndExtracted -Url $archiveAsset.browser_download_url -DestinationDir $DllVersionDir -ExpectedFile "nvngx.dll"
